...
- `packages/desktop-client/src/components/mobile/budget/BudgetPage.tsx` — call hook, pass to `onOpenBudgetPageMenu`; add `onPayPeriodConfigChange` useEffectEvent to reconnect bounds on toggle (see mobile D7)
...
- Mobile short label format / `(PPX)` suffix removal (see `mobile-pay-period-enhancements`)
